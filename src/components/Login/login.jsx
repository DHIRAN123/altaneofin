...
const Login = () => {
  ...
  const onSendOTP = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://ec2-35-154-147-165.ap-south-1.compute.amazonaws.com:8080/sendOtp?mobileNumber=${phone}`, {
        method: 'GET',
  ...
  const onVerifyOTP = async (e) => {
    ...
    if (data.otp.length < 6) {
      ...
      const OTP = data.otp
      const response = await fetch(`http://ec2-35-154-147-165.ap-south-1.compute.amazonaws.com:8080/verifyOtp?otp=${data.otp}`, {
        method: 'POST',
        headers: { 'Content-type': 'application/json' }
      })
      const res = await response.json()
